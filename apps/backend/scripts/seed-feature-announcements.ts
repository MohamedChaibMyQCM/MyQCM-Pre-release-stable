...
// Seed logic copied here so it doesn't get compiled by Nest CLI
async function seedFeatureAnnouncements(dataSource: DataSource) {
  const featureAnnouncementRepo = dataSource.getRepository('feature_announcement' as any);
  ...
  for (const announcementData of sampleAnnouncements) {
    // Use plain query builder to avoid entity typing issues
    const result = await featureAnnouncementRepo.insert({
      ...announcementData,
      createdAt: new Date(),
      updatedAt: new Date(),
      version: announcementData.version || null,
    } as any);
    createdAnnouncements.push(announcementData);
    console.log(`✓ Created: ${announcementData.title}`);
  }
...
if (require.main === module) {
  (async () => {
    try {
      ...
      await seedFeatureAnnouncements(AppDataSource as any);
