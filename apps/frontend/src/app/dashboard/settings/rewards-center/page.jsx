...
const RewardsCenter = () => {
  ...
  if (!token) {
    return (
      <div className="mx-5 my-16 rounded-[20px] border border-dashed border-[#F8589F]/40 bg-white p-10 text-center shadow-sm">
        <h2 className="text-[20px] font-semibold text-[#191919] mb-3">
  ...
  return (
    <div className="mx-5 mb-20 mt-8">
      <motion.div
        className="mb-8 rounded-[20px] border border-[#FFE5F1] bg-gradient-to-br from-white to-[#FFF5FA] p-8 shadow-[0px_2px_12px_rgba(248,88,159,0.08)]"
      ...
      >
        <div className="flex items-center justify-between max-md:flex-col max-md:items-start max-md:gap-6">
          <div>
            <h1 className="mb-3 text-[24px] font-[600] text-[#191919]">
              Centre de Récompenses
            </h1>
            <p className="max-w-[600px] text-[14px] leading-relaxed text-gray-600">
              Convertissez vos XP en avantages concrets : expériences
              professionnelles, boosts d’apprentissage et privilèges exclusifs.
            </p>
          </div>
          <div className="rounded-[16px] border-2 border-[#FFE5F1] bg-white px-8 py-5 text-center shadow-[0px_4px_16px_rgba(248,88,159,0.12)]">
            <p className="mb-1 text-[12px] font-[500] text-gray-500">
              Votre Solde XP
            </p>
          ...
          <button
            ...
            className={`whitespace-nowrap rounded-[16px] px-5 py-3 text-[13px] font-[500] transition-all duration-300 ${
              selectedTab === tab.id
                ...
                : "border border-gray-100 bg-white text-[#191919] hover:shadow-md"
            }`}
          >
            {tab.label}
          </button>
      ...
      {selectedTab === "opportunites" && (
        ...
        >
          <h2 className="mb-6 text-[18px] font-[600] text-[#191919]">
            Enchères XP - Opportunités Professionnelles
          </h2>

          {isLoadingAuctions ? (
            <p className="rounded-[16px] border border-dashed border-[#F8589F]/30 bg-white px-6 py-8 text-center text-[14px] text-gray-600">
              Chargement des opportunités…
            </p>
          ) : auctions.length === 0 ? (
            <div className="rounded-[16px] border border-dashed border-[#F8589F]/30 bg-white px-6 py-12 text-center shadow-sm">
              <h3 className="mb-2 text-[16px] font-semibold text-[#191919]">
            ...
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-3">
              {auctions.map((auction, index) => {
                ...
                return (
                  <motion.div
                    key={auction.id}
                    className="relative overflow-hidden rounded-[18px] border border-gray-100 bg-white p-6 shadow-[0px_2px_12px_rgba(0,0,0,0.06)] transition-all duration-300 hover:border-[#FFE5F1] hover:shadow-[0px_8px_24px_rgba(248,88,159,0.15)]"
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: index * 0.1 }}
                    whileHover={{ y: -4 }}
                  >
                    <div className="absolute top-0 right-0 h-24 w-24 rounded-bl-[100px] bg-gradient-to-br from-[#FFF5FA] to-transparent opacity-50" />
                    ...
                    <h3 className="relative z-10 mb-2 text-[15px] font-[600] text-[#191919]">
                      {auction.title}
                    </h3>
                    <p className="relative z-10 mb-4 text-[13px] text-gray-600">
                      {auction.partner || "Partenaire à confirmer"}
                    </p>

                    <div className="relative z-10 mb-4 rounded-[12px] bg-gradient-to-r from-[#FFF5FA] to-white p-3">
            ...
            <button
              ...
              className={`whitespace-nowrap rounded-full px-4 py-2 text-[12px] font-semibold transition-colors ${
                selectedCategoryId === "all"
                  ...
                  : "border border-gray-200 bg-white text-gray-600 hover:border-[#F8589F]/40"
              }`}
            >
              Toutes les catégories
            </button>
            {isLoadingCategories ? (
              <span className="rounded-full border border-dashed border-[#F8589F]/40 px-4 py-2 text-[12px] text-gray-500">
                Chargement…
              </span>
            ) : (
            ...
            <div className="grid grid-cols-1 gap-5 md:grid-cols-2 lg:grid-cols-4">
              {filteredPerks.map((perk, index) => {
                ...
                return (
                  <motion.div
                    key={perk.id}
                    className={`relative flex flex-col overflow-hidden rounded-[20px] border-2 border-gray-50 bg-white p-6 shadow-[0px_2px_12px_rgba(0,0,0,0.06)] transition-all duration-500 hover:shadow-[0px_12px_32px_rgba(0,0,0,0.12)] ${
                  ...
                  >
                    ...
                    {perk.category?.name && (
                      <span className="relative z-10 mb-4 inline-flex w-fit items-center rounded-full bg-[#FFF5FA] px-3 py-1 text-[11px] font-semibold text-[#F8589F]">
                        {perk.category.name}
                      </span>
                    )}
                    ...
                    <h3 className="relative z-10 mb-2 text-[15px] font-[600] leading-snug text-[#191919]">
                      {perk.title}
                    </h3>
                    <p className="relative z-10 mb-4 flex-1 text-[12px] leading-relaxed text-gray-600">
                      {perk.description || "Un avantage exclusif pour les membres."}
                    </p>

                    <div className="relative z-10 mb-4 flex flex-wrap gap-3 text-[11px] text-gray-500">
                      {perk.remainingStock !== null && (
                        <span
                          className={`rounded-full bg-gray-100 px-3 py-1 font-semibold ${
                    ...
                    <div className="relative z-10 mb-5 rounded-[14px] border border-[#FFE5F1] bg-gradient-to-r from-[#FFF5FA] via-white to-[#FFF5FA] p-4 text-center shadow-sm">
      ...
      {selectedTab === "soutien" && (
        <motion.div
          ...
          transition={{ duration: 0.4 }}
        >
          <div className="rounded-[20px] border border-dashed border-[#F8589F]/30 bg-white px-8 py-12 text-center shadow-sm">
            <h2 className="mb-3 text-[18px] font-[600] text-[#191919]">
              Soutien Académique
            </h2>
            <p className="text-[13px] text-gray-600">
              Nous finalisons l’intégration des services de tutorat premium. Vos
      ...
      <motion.div
        className="mt-16 rounded-[20px] border border-gray-100 bg-white p-6 shadow-[0px_2px_12px_rgba(0,0,0,0.06)]"
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.4 }}
      >
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-[18px] font-[600] text-[#191919]">
        ...
        {isLoadingTransactions ? (
          <p className="rounded-[16px] border border-dashed border-[#F8589F]/30 px-6 py-6 text-center text-[13px] text-gray-600">
            Chargement de vos transactions…
          </p>
        ) : transactions.length === 0 ? (
          <div className="rounded-[16px] border border-dashed border-[#F8589F]/30 px-6 py-8 text-center text-[13px] text-gray-600">
