...
const SidebarSection = ({
  ...
  >
    <div
      className="mb-2 inline-flex items-center gap-2 rounded-md px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.18em]"
      style={{
        color: COLORS.primaryDark,
    ...
    >
      {Icon ? <Icon className="h-3.5 w-3.5" style={{ color: COLORS.primary }} /> : null}
      {title}
    </div>
    {children}
  </section>
...
export const CaseSidebar = ({
  ...
  return (
    <aside className="w-full max-w-[360px] md:max-w-[380px] lg:sticky lg:top-6">
      <motion.div
        initial={{ opacity: 0, y: 8 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.45, ease: [0.22, 1, 0.36, 1] }}
        className="rounded-[20px] border px-6 py-6 lg:max-h-[calc(100vh-8rem)] lg:overflow-y-auto"
        style={{
          background: "#FFFFFF",
          borderColor: "#F0E4EC",
      ...
      >
        ...
        <h2
          className="mt-2 text-[18px] font-semibold leading-snug md:text-[20px]"
        ...
        {displayMeta.length > 0 ? (
          <div className="mt-3 flex flex-wrap gap-2.5">
            {displayMeta.map((chip) => (
              <Pill key={chip}>
                <Tag className="h-3.5 w-3.5 -translate-y-[1px]" style={{ color: COLORS.primary }} />
                {chip}
              </Pill>
            ))}
          </div>
        ...
        <div className="mt-5 grid gap-4">
          <SidebarSection title="Scenario" icon={BookOpenText}>
            <p className="text-[13px] leading-relaxed text-[#5F6676]">
              {scenario}
            </p>
          </SidebarSection>
          ...
          {objectives.length > 0 ? (
            <SidebarSection title="Objectifs" icon={Bookmark}>
              <ul
                className="space-y-1.5 text-[13px] leading-relaxed text-[#5F6676]"
              >
