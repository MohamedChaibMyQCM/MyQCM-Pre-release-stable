...
import { QROCForm, QROCData } from "@/components/forms/QROCForm";
...
export default function QROCManualCreatePage() {
  ...
  const [qrocData, setQrocData] = React.useState<QROCData>({
    question: "",
    answer: "",
  });
  ...
  const validate = (): string | null => {
    ...
    if (!qrocData.question.trim()) {
      return "Please enter a question";
    }

    if (!qrocData.answer.trim()) {
      return "Please enter the expected answer";
    }
      ...
      <div className="mx-auto max-w-4xl">
        <div className="mb-8">
          <h1 className="text-3xl font-bold tracking-tight">
            Create QROC Manually
          </h1>
          <p className="text-muted-foreground">
            Create a short answer question from scratch with full control
          </p>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-lg border bg-card p-6">
            <h2 className="mb-4 text-lg font-semibold">Question & Answer</h2>
            <QROCForm value={qrocData} onChange={setQrocData} />
          </div>
